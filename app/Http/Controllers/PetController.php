...
class PetController extends Controller
{
    ...
    {
        $pets = Pet::with('photos')->orderBy('name')->get();
        $petsData = $pets->map(function($p){
            return [
                'id' => $p->id,
                'name' => $p->name,
                'type' => $p->animal_type,
                'photo' => ($p->photos->first() ? asset('storage/'.$p->photos->first()->path) : null),
                'services' => $p->services ?? ($p->service_type ? [$p->service_type] : []),
            ];
        })->values()->all();
        return view('admin.pets.calendar', compact('pets','petsData'));
    }
    ...
    public function calendarData(Request $request)
    {
        ...
        $entries = \App\Models\PetCalendarEntry::with(['pet.photos'])
            ...
            ->map(function($e){
                return [
                    ...
                    'pet_id' => $e->pet_id,
                    'pet_name' => $e->pet?->name,
                    'pet_type' => $e->pet?->animal_type,
                    'pet_photo' => $e->pet && $e->pet->photos->first() ? asset('storage/'.$e->pet->photos->first()->path) : null,
                    'service' => $e->service,
                    'slot' => $e->slot,
                ];
            });
    ...
    // Toggle entries for a day for selected pet, service and slots
    public function calendarToggle(Request $request)
    {
        $data = $request->validate([
            'date' => 'required|date',
            'pet_id' => 'required|exists:pets,id',
            'service' => 'required|in:передержка,выгул,кормление',
            'slots' => 'required|array|min:1',
            'slots.*' => 'required|in:утро,день,вечер',
        ]);
        $created = []; $deleted = [];
        foreach ($data['slots'] as $slot) {
            $existing = \App\Models\PetCalendarEntry::where([
                'pet_id' => $data['pet_id'],
                'date' => $data['date'],
            ...
            if ($existing) {
                ...
                $created[] = \App\Models\PetCalendarEntry::create([
                    'pet_id' => $data['pet_id'],
                    'date' => $data['date'],
                    'service' => $data['service'],
                    'slot' => $slot,
        ...
        return response()->json(['ok' => true, 'created' => $created, 'deleted' => $deleted]);
    }
}
