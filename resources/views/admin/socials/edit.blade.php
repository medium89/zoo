...
<form action="{{ route('admin.socials.update', $social->id) }}" method="POST">
    ...
    <div class="mb-3">
        ...
        <!-- Скрытое поле для хранения значения иконки -->
        <input type="hidden" id="icon" name="icon" value="{{ $social->icon }}" required>
        
        <!-- Кнопка для открытия модального окна выбора иконок -->
        <button type="button" class="btn btn-secondary" id="selectIconButton">Выбрать иконку</button>
        
        <!-- Отображение текущей выбранной иконки -->
    ...
    <!-- Модальное окно для выбора иконок (пока пустое, будет заполнено позже JS) -->
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Список популярных иконок Font Awesome (можно расширить)
        const icons = [
            ...
            'fas fa-link', 'fas fa-share-alt', 'fas fa-comments', 'fas fa-heart', 'fab fa-odnoklassniki'
        ];

        function renderIcons(filter = '') {
            iconList.innerHTML = ''; // Очищаем список
        ...
        // Инициализация отображения иконок при загрузке
        renderIcons();

        // Открытие модального окна
        selectIconButton.addEventListener('click', () => {
            iconPickerModal.style.display = 'block';
            iconSearchInput.value = ''; // Очистить поиск при открытии
            renderIcons(); // Рендерить все иконки
        });
