...
<div class="modal fade admin-modal" id="mapNodeDetailsModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable map-form-dialog">
        <div class="modal-content map-form-modal">
            ...
            <div class="modal-body map-form-modal__body" id="mapNodeDetailsBody"></div>
            <div class="modal-footer map-form-modal__footer"><button type="button" class="btn btn-light" data-bs-dismiss="modal">Закрыть</button><button class="btn btn-primary" type="button" id="mapNodeDetailsEdit"><i class="fa fa-pen me-1"></i>Редактировать</button></div>
        </div>
    </div>
</div>
@endsection
...
.map-form-modal__header{padding:17px 20px;border-color:#e8edf3}
.map-form-modal .modal-title{font-size:1.05rem}
.map-form-modal__body{padding:18px 20px 22px}
.map-form-modal__lead{margin-bottom:16px;font-size:.84rem}
.map-form-field{margin-top:13px}
.map-form-field label{margin-bottom:6px;color:#607288;font-size:.78rem}
.map-form-field .form-control,.map-form-field .form-select{min-height:40px;border-radius:8px;font-size:.88rem}
.map-form-modal__footer{padding:14px 20px 18px;border-color:#e8edf3;background:#fbfcfe}
.map-form-modal__footer .btn{min-height:39px;padding:8px 13px;font-size:.84rem}
@media(max-width:575px){.map-form-dialog{margin:8px}.map-form-modal__header{padding:15px 16px 13px}.map-form-modal__body{padding:16px}.map-form-modal__footer{padding:12px 16px 15px}}
</style>
@endpush
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const nodeDetailsModal = document.getElementById('mapNodeDetailsModal');
    ...
    const linkPath = (animal, client) => {
        ...
        const sx = animal.x + (horizontalDirection > 0 ? 248 : 0);
        const sy = animal.y + 64;
        const tx = client.x + (horizontalDirection > 0 ? 0 : 248);
    ...
    const renderLinks = () => {
        ...
        if (linking) {
            const startX = linking.node.x + 124;
    ...
    const openNodeDetails = node => {
        viewedNode = node;
        const isClient = node.type === 'client';
        const title = document.getElementById('mapNodeDetailsTitle');
        const eyebrow = document.getElementById('mapNodeDetailsEyebrow');
        const body = document.getElementById('mapNodeDetailsBody');
        if (isClient) {
            const pets = animals.filter(animal => animal.client_id === node.id);
            eyebrow.innerHTML = '<i class="fa fa-user"></i> Клиент'; title.textContent = node.name;
            body.innerHTML = `<div class="map-node-details__hero"><div class="map-node-details__photo"><i class="fa fa-user"></i></div><div><div class="map-node-details__name">${escapeHtml(node.name)}</div><div class="map-node-details__subtitle">Карточка клиента</div></div></div><div class="map-node-details__list"><div class="map-node-details__row"><span class="map-node-details__label">Телефон</span><span class="map-node-details__value">${escapeHtml(node.phone || 'Не указан')}</span></div><div class="map-node-details__row"><span class="map-node-details__label">Адрес</span><span class="map-node-details__value">${escapeHtml(node.address || 'Не указан')}</span></div><div class="map-node-details__row"><span class="map-node-details__label">Питомцы</span><span class="map-node-details__value">${pets.length || 'Нет'}</span></div></div>${pets.length ? `<div class="map-node-details__pets mt-3">${pets.map(pet => `<span class="map-node-details__pet">${escapeHtml(pet.name)}</span>`).join('')}</div>` : ''}`;
        } else {
            const category = categories.find(item => Number(item.id) === Number(node.category_id));
            const client = clients.find(item => item.id === node.client_id);
            const photo = node.photo ? `<img class="map-node-details__photo" src="${escapeHtml(node.photo)}" alt="">` : '<div class="map-node-details__photo">🐾</div>';
            eyebrow.innerHTML = '<i class="fa fa-paw"></i> Питомец'; title.textContent = node.name;
            body.innerHTML = `<div class="map-node-details__hero">${photo}<div><div class="map-node-details__name">${escapeHtml(node.name)}</div><div class="map-node-details__subtitle">Карточка питомца</div></div></div><div class="map-node-details__list"><div class="map-node-details__row"><span class="map-node-details__label">Категория</span><span class="map-node-details__value">${escapeHtml(category?.name || 'Не указана')}</span></div><div class="map-node-details__row"><span class="map-node-details__label">Хозяин</span><span class="map-node-details__value">${escapeHtml(client?.name || 'Не указан')}</span></div></div>`;
        }
        bootstrap.Modal.getOrCreateInstance(nodeDetailsModal).show();
    };
    document.getElementById('mapNodeDetailsEdit').addEventListener('click', () => {
        const node = viewedNode;
        if (!node) return;
        nodeDetailsModal.addEventListener('hidden.bs.modal', () => openNodeEditor(node), {once: true});
        bootstrap.Modal.getOrCreateInstance(nodeDetailsModal).hide();
    });
